function [C1new, dsC1new, C2new, Cnew, dsCnew, Xnew, Psinew, Znew, Unew, dsUnew, zetaNew, dszetaNew, zetacNew, dszetacNew, zetanemNew, dszetanemNew, zetacnemNew, dszetacnemNew, xintegral, solnem, s0new, s0inv, Q] = evolvefunctions_t0(svec, zeta, dszeta, zeta_const, zeta_la, zeta_sigma, zeta_par, zetac, dszetac, zetac_const, zetac_la, zetac_sigma, zetac_par, zetanem, dszetanem, zetacnem, dszetacnem, zetanem_Profile, zetanem_const, zetanem_la, zetanem_sigma, zetanem_par, zetacnem_Profile, zetacnem_const, zetacnem_la, zetacnem_sigma, zetacnem_par, vs, dsvs, vkk, tss, vn, dsvn, mss, tns, ds2vn, U, C1, dsC1, C2, C, dsC, kappa, X, Psi, Z, snewfun, snewvec, sfun, t, dt, thalf, tsigma, L, Lnew, L0, eps1, eps2, etacb, npoints, dsw0, dswL, lc, Q, dsQ, s0, optode_nem)
%% Lagrangian time evolution of the surface descriptors and fields, for first time step t=dt: 
    ...
    sgrid = snew;
    sgridnem = [0. snew((snew > eps1)&(snew < (Lnew-eps2))) Lnew];
    ...
    %% Solve nematic Euler-Lagrange equations, with solution on the sphere as initial guess:
    parguess = [dsw0 dswL];
    ...
    switch zetanem_Profile
        case 'Gaussian'
            zetanemvec = Qnew.*superGaussian(s0new(sgridnem), zetanem_la*L0, zetanem_par, zetanem_const, zetanem_sigma*L0, 1);
        case 'Sigmoidal'
            if zetanem_la==1
                zetanemvec = zetanem_const*Qnew;
            else
                zetanemvec = zetanem_const*Qnew+zetanem_par*Qnew.*sigmoidal(s0new(sgridnem), zetanem_la*L0, zetanem_sigma*L0);
            end
    end
    ...
    switch zetacnem_Profile
        case 'Gaussian'
            zetacnemvec = Qnew.*superGaussian(s0new(sgridnem), zetacnem_la*L0, zetacnem_par, zetacnem_const, zetacnem_sigma*L0, 1);
        case 'Sigmoidal'
            if zetacnem_la==1
                zetacnemvec = zetacnem_const*Qnew;
            else
                zetacnemvec = zetacnem_const*Qnew+zetacnem_par*Qnew.*sigmoidal(s0new(sgridnem), zetacnem_la*L0, zetacnem_sigma*L0);
            end
    end
