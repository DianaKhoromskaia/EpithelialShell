...
if ~SolFound
    % --- minimal safe outputs so caller doesn't crash ---
    sint = sgrid;                 % return the grid we attempted
    Vint = NaN(10, numel(sint));  % 10 equations
    Par  = NaN(1,2);              % 2 parameters (P or dVdt, and fc)
    vs = []; dsvs = []; vkk = []; tss = [];
