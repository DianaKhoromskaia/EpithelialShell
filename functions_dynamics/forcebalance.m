...
tss = griddedInterpolant(sint, 2*K*U(sint)+zeta(sint)-zetanem(sint)-(2*kappa(sint).*(C(sint)-C0)+zetac(sint)).*(C2(sint)-0.5*C(sint))-(kappa(sint)*C0-0.5*zetac(sint)).*C(sint)+(etab+eta)*Vint(1,:)+(etab*C(sint)+eta*(C2(sint)-C1(sint))).*Vint(2,:)+(etab-eta)*[Vint(1,1) cos(Psi(sint(2:end))).*Vint(8,2:end)./X(sint(2:end))], 'spline');
