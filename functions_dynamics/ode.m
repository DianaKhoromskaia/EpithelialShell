function dvds = ode(s, v, varpar, U, dsU, C1, C2, C, C0, dsC, Psi, X, Z, X0, L, L0, zeta, dszeta, zetac, dszetac, zetanem, dszetanem, zetacnem, eta, etab, etacb, etap, xintegral, fext, kappa, dskappa, K, xi, kb, x0b, kp, psi0b, FixedPar, t, P0, thalf_P, tsigma, zc0_on)
...
tss = 2*K*U(s) + zeta(s) - zetanem(s) -(kappa(s).*(C(s)-C0)+zc0_on*0.5*zetac(s)).*(C2(s)-C1(s)) + zc0_on*(0.5*zetac(s)-C0*kappa(s)).*(C(s)-C0+0.5*zetac(s)./kappa(s)) + (eta+etab)*dsvs + (etab-eta)*cos(Psi(s)).*vs./X(s) + (etab*C(s)+eta*(C2(s)-C1(s))).*vn;
...
% specify force balance at SP:
        ...
        dvds(:,indices) =   [(-dszeta(indices) + zc0_on*dszetac(indices).*C1(indices) + (zc0_on*0.5*zetac(indices)./kappa(indices)-C0).*(zc0_on*dszetac(indices)+(C0-zc0_on*0.5*zetac(indices)./kappa(indices)).*dskappa(indices)./kappa(indices)))/(eta+etab);%-C2(0).*tns(indices);%zervec;
            ...
            tns(indices);
            -0.5*P*onevec+C2(indices).*(2*K*U(indices) + zeta(indices) + (0.5*zetac(indices)-kappa(indices)*C0).*(C(indices)-C0+0.5*zetac(indices)./kappa(indices)) + (eta+etab)*dsvs(indices) + etab*C(indices).*vn(indices))+0.5*xi*vn(indices)+0.5*fc*onevec;%-0.5*P*onevec+C2(0).*tss(indices)+0.5*xi*vn(indices)+0.5*fc*onevec;
            zervec;
            zervec;
