...
% specify force balance at SP:
        ...
        dvds(:,indices) =   [(-dszeta(0) + dszetac(0)*C2(0) + 2*dskappa(0)*C0)/(eta+etab);%-C2(0).*tns(indices);%zervec;
            zervec;
            0.5*(-vn(indices).*(C1(0).^2 + C2(0).^2) - (mss(indices) - 2*kappa(0)*(C(0)-C0) - zetac(0))/etacb);
            tns(indices);
            -0.5*P*onevec+C2(0).*(2*K*U(0) + zeta(0) - zetanem(0) - (2*kappa(0)*(C(0)- C0) + zetac(0))*(C2(0)-0.5*C(0)) + (0.5*zetac(0)-kappa(0)*C0)*(C(0)-C0+0.5*zetac(0)/kappa(0)) + (eta+etab)*dsvs(indices) + etab*C(0)*vn(indices))+0.5*xi*vn(indices)+0.5*fc*onevec;%-0.5*P*onevec+C2(0).*tss(indices)+0.5*xi*vn(indices)+0.5*fc*onevec;
            zervec;
            zervec;
