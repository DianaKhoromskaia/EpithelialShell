...
% force balance at SP:
        ...
        dvds(:,indices) =   [zervec;
            ...
            tns(indices);
            -0.5*P*onevec+C2(0).*(2*K*U(0) + zeta(0) - zetanem(0) + 2*etab*dsvs(indices) + etab*C(0)*vn(indices))+0.5*xi*vn(indices)+0.5*fc*onevec;
            zervec;
            zervec;
            dsvs(indices);
 ...
 % force balance at NP:       
        ...
        dvds(:,indices) =   [zervec;
            ...
            tns(indices);
            -0.5*P*onevec+C2(L).*(2*K*U(L) + zeta(L) - zetanem(L) + 2*etab*dsvs(indices) + etab*C(L)*vn(indices))+0.5*xi*vn(indices)-0.5*fc.*onevec;
            zervec;
            zervec;
            dsvs(indices);
