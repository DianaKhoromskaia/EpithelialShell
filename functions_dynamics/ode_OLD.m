...
% additional term for ds2vs when including etacb*DCkk/Dt
%ds2vs_DCkk_term = etacb*cos(Psi(s)).*(DCkk)./X(s) - (tns + 2*cos(Psi(s)).*zetacnem(s)./X(s) - 2*dskappa(s).*(C(s)-C0) - 2*kappa(s).*dsC(s) - dszetac(s) + dszetacnem(s)).*C2(s) - etacb*DCkk.*(dsC(s) - dsC1(s));
etacb_DC_on = 0;
ds2vs_DCkk_term = 0;
% end test
...
ds2vn = -cos(Psi(s)).*dsvn./X(s) - vn.*(C1(s).^2 + C2(s).^2) + vs.*dsC(s) - DCkk;
...
dstns = 2*C1(s).*(T+V+(kappa(s).*(C(s)-C0)+zc0_on*0.5*zetac(s)).*(C2(s)-C1(s))+etacb_DC_on*etacb*DCkk.*(C1(s)-C2(s))) + C(s).*tss - cos(Psi(s)).*tns./X(s) - P + cos(Psi(s)).*fc  - fextn;
...
% NEED TO DERIVE THE POLE LIMITS
